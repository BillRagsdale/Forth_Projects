...
\ 2021-03-12 -I Release as Lesson One.
\
...
: .game  ( --- ) \ display the board with Xs, Os and numbers.
  1 3numbers dashes 3numbers dashes 3numbers drop ;
...
: player-input ( --- boolean ) \ false if a play, true is excape to end.
 ...
 BEGIN
   \ .board  \ clear and display board
    ."    Square number for " current-player
    if ." X:  " else ." O:  " then
    key dup emit cr
    dup  27 ( esc ) =
     ...
     ASCII># dup  range?
           over empty? and
     if   place-symbol ( .game ) cr 0  exit then
   ( otherwise )  drop  ."    Pick another square.  " cr
 AGAIN  ;
...
: full-game ( --- ) \ user game without scoring.
  start  cr cr ." Enter 'esc' to exit. "
  BEGIN cr .game player-input if exit then \ exit on true
        unplayed 0= UNTIL ;
